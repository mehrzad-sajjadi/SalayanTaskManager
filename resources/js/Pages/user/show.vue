<template>
    ...
    <AuthenticatedLayout>
        ...
        <div class="w-5/6 mx-auto my-16 border border-gray-500 rounded-lg">
            ...
            >
                <p class="px-4 m-2 text-xl">لیست تسک های {{ user.name }}</p>
                <div class="flex flex-row">
                    <Link
                        :href="route('task_create', user.id)"
                        as="button"
                        type="button"
        ...
        <p
            class="flex mb-15 flex-row justify-center text-xl text-center"
...
if (usePage().props.crud.success) {
    alert(usePage().props.crud.success);
}

function remove(id) {
    if (confirm("آیا از حذف تسک مطمئن هستید ؟ ")) {
        router.delete(route("task_delete", id));
    }
}
</script>
